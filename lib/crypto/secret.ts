...
export async function encryptAesGcm(keyBytes: Uint8Array, plaintext: Uint8Array, aad?: Uint8Array) {
  ...
  const enc = await crypto.subtle.encrypt({ name: "AES-GCM", iv, additionalData: aad as BufferSource | undefined }, key, plaintext as BufferSource);
...
export async function decryptAesGcm(keyBytes: Uint8Array, iv: Uint8Array, ciphertext: Uint8Array, aad?: Uint8Array) {
  ...
  const dec = await crypto.subtle.decrypt({ name: "AES-GCM", iv, additionalData: aad as BufferSource | undefined }, key, ciphertext as BufferSource);
